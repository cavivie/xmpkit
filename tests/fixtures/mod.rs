...
//! Test fixtures for xmpkit
//!
//! This module provides test data files and helper functions for tests.
//!
//! ## Directory Structure
//!
//! ```
//! tests/fixtures/
//! ├── mod.rs          # This module (helper functions and XMP examples)
...
//! ## Usage
//!
//! - **Data files**: Use `fixture_path("filename")` to get the path to a file in `data/`
//! - **XMP examples**: Use constants like `PURPLE_SQUARE_XMP`, `STRUCT_EXAMPLE`, etc.

#![allow(dead_code)] // Test fixtures may not all be used in every test run
...
///
/// Returns a `PathBuf` pointing to the fixture file in `tests/fixtures/data/`.
...
/// Create a temporary copy of a fixture file
///
/// Copies the fixture file to the specified temporary directory and returns
/// the path to the copy as a string.
pub fn temp_copy_of_fixture(tempdir: &Path, name: &str) -> String {
    let fixture_src = fixture_path(name);
    let fixture_dest = Path::join(tempdir, name);
    fs::copy(&fixture_src, &fixture_dest).unwrap();
    fixture_dest.display().to_string()
}

/// XMP metadata from Purple Square.psd fixture file
pub const PURPLE_SQUARE_XMP: &str = r#"<x:xmpmeta xmlns:x="adobe:ns:meta/" x:xmptk="Adobe XMP Core 4.0-c003 (debug), build -num-, -date-">
...
/// Example XMP with structure properties
pub const STRUCT_EXAMPLE: &str = r#"
...
/// Example XMP with array properties
pub const ARRAY_EXAMPLE: &str = r#"
        <x:xmpmeta xmlns:x="adobe:ns:meta/" x:xmptk="Adobe XMP Core 7.0-c000 1.000000, 0000/00/00-00:00:00">
        <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
        <rdf:Description rdf:about=""
        xmlns:dc="http://purl.org/dc/elements/1.1/">
...
/// Example XMP with qualifier properties
pub const QUAL_EXAMPLE: &str = r#"
        <rdf:RDF xmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#'>
...
/// Example XMP with localized text properties
pub const LOCALIZED_TEXT_EXAMPLE: &str = r#"<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
        xmlns:dc="http://purl.org/dc/elements/1.1/">
        <rdf:Description rdf:about="">
            <dc:title>
                <rdf:Alt>
...
/// Example RDF with newline characters
pub const NEWLINE_RDF: &str = "<rdf:RDF xmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#'>
...
/// Example RDF with inconsistent property names
pub const INCONSISTENT_RDF: &str = r#"<rdf:RDF xmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#'>
...
/// Comprehensive RDF coverage example with various property types
pub const RDF_COVERAGE: &str = r#"<rdf:RDF xmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#'>                
        <rdf:Description rdf:about='Test:XMPCoreCoverage/kRDFCoverage' xmlns:ns1='ns:test1/' xmlns:ns2='ns:test2/'>              
